...
sgs.ai_skill_playerchosen.tuxi = function(self)
    ...
    if lord and self:isEnemy(lord) and sgs.turncount <= 1 and not lord:isKongcheng() then
        if add_player(lord) == tuxi_mark then return targets end
    end
    ...
    if jiangwei and self:isFriend(jiangwei) and jiangwei:getMark("zhiji") == 0 and jiangwei:getHandcardNum()== 1
        ...
        if add_player(jiangwei,1) == tuxi_mark  then return targets end
    end
    ...
    if zhijiangwei and self:isFriend(zhijiangwei) and zhijiangwei:getHandcardNum()== 1 and
        ...
        if isGood and add_player(zhijiangwei, 1) == tuxi_mark  then return targets end
    end
    ...
    for i = 1, #self.enemies, 1 do
        ...
        for _, card in ipairs(cards) do
            if (card:hasFlag("visible") or card:hasFlag(flag)) and (card:isKindOf("Peach") or card:isKindOf("Nullification") or card:isKindOf("Analeptic") ) then
                if add_player(p)==tuxi_mark  then return targets end
            end
        end
    end
    ...
    for i = 1, #self.enemies, 1 do
        ...
        if good_target and add_player(p)==tuxi_mark then return targets end
    end
...
sgs.ai_skill_playerchosen.fenwei = function(self)
	
	local liuxie = self.room:findPlayerBySkillName("huangen")
    if liuxie and self:isFriend(liuxie) and not self:isWeak(liuxie) then return {} end

	local use = self.player:getTag("fenwei"):toCardUse()
    local card = use.card
    local from = use.from
    ...
    for _, player in ipairs(players) do
        if not (player:hasSkill("danlao") or (player:hasSkills("jianxiong|nosjianxiong") and not self:isWeak(player))) and targets:contains(player) then
