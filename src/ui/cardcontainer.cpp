...
void CardContainer::startPending(const ViewAsSkill *skill)
{
    confirm_button->hide();
    foreach (CardItem *item, items) {
        connect(item, &CardItem::clicked, this, &CardContainer::onItemClicked);
    }
    filterCards(skill);
}

void CardContainer::filterCards(const ViewAsSkill *skill)
{
    QList<const Card *> cards;
    QList<CardItem *> pendings = RoomSceneInstance->getDasboard()->getPendings();
    foreach(CardItem *item, pendings)
        cards.append(item->getCard());
    foreach (CardItem *item, items) {
        item->setEnabled(item->isChosen() || skill->viewFilter(cards, item->getCard()));
    }
}
...
void CardContainer::onItemClicked()
{
    CardItem *card_item = qobject_cast<CardItem *>(sender());
    if (card_item) {
        card_item->setChosen(!card_item->isChosen());
        RoomSceneInstance->getDasboard()->externalPending(card_item);
