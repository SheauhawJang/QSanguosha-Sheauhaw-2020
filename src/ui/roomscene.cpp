    ...
    : main_window(main_window), m_tableBgPixmap(1, 1), m_tableBgPixmapOrig(1, 1), game_started(false)
{
    ...
    m_yanjiaoBox = new YanjiaoBox;
    m_yanjiaoBox->hide();
    addItem(m_yanjiaoBox);
    m_yanjiaoBox->setZValue(20000.0);

    connect(ClientInstance, SIGNAL(yanjiaochoose(QList<int>, QString, QString)), m_yanjiaoBox, SLOT(doYanjiaoChoose(QList<int>, QString, QString)));
...
void RoomScene::updateTable()
{
    ...
    m_yanjiaoBox->setPos(m_tableCenterPos - QPointF(m_yanjiaoBox->boundingRect().width() / 2, m_yanjiaoBox->boundingRect().height() / 2));
...
void RoomScene::enableTargetsAfterSelect()
{
    const Card *card = dashboard->getSelected();

    if (card && (card->targetFixed() || card->targetsFeasible(selected_targets, Self))) {
...
void RoomScene::doTimeout()
{
    ...
    case Client::AskForYanjiao: {
        QList<int> empty;
        m_yanjiaoBox->clear();
        ClientInstance->onPlayerReplyYanjiao(empty, empty);
        break;
    }
    case Client::AskForGuanxing:
    case Client::AskForGongxin: {
    ...
    case Client::Responding: {
        ...
        if (rx.exactMatch(pattern)) {
            ...
            if (skill) {
                ...
                if (skill->getExpandPile() == "cardcontainer") {
                    m_cardContainer->startPending(skill);
...
void RoomScene::filterCardContainer(const ViewAsSkill *skill)
{
    m_cardContainer->filterCards(skill);
}
