        ...
        do {
            ...
            foreach (const TriggerSkill *skill, skills) {
                if (!triggered.contains(skill)) {
                    if (skill->objectName() == "game_rule" || (Config.EnableBasara && skill->objectName() == "basara_mode") || (room->getScenario()
                    ...
                    forever {
                        ...
                        if (name.isEmpty()) {
                            if (p != NULL) {
                                ...
                                if (skill_names.length() == 1 || !Config.EnableTriggerOrder)
                                    name = names.first();
                                else {
                        ...
                        foreach (const QString &s, already_triggered) {
                            ...
                            foreach (const QString &skill_name, who_skills) {
                                ...
                                if (_skillname.endsWith("!"))
                                    _skillname.chop(1);
                                if (_skillname == _s) {
                                    trigger_who[p].removeOne(skill_name);
                                    break;
                                }
                            }
                        }
