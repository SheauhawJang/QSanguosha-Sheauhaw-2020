...
void ServerPlayer::fillHandCards(int n, const QString &reason, bool from_up)
{
    if (n > getHandcardNum())
        drawCards(n - getHandcardNum(), reason, from_up);
}
...
QList<ServerPlayer *> ServerPlayer::getCardDefautTargets(const Card *card, bool distance_limited)
{
    ...
    if (!distance_limited)
        room->setCardFlag(card, "Global_NoDistanceChecking");
...
void ServerPlayer::changeLesbianSkill(const QString &skill, bool hidden)
{
    ...
    if (player && player->hasSkill(skill))
    {
        ...
        QString new_skill = skill + Skill::Lesbian();
        room->acquireSkill(player, new_skill);
    }
}
