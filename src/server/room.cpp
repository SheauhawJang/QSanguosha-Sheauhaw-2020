...
QList<int> Room::getNCards(int n, bool update_pile_number, bool from_up)
{
    ...
    if (update_pile_number) {
        ...
        thread->trigger(ViewDrawPileCard, this, NULL);
    }
...
bool Room::askForSkillInvoke(ServerPlayer *player, const QString &skill_name, const QVariant &data)
{
    ...
    if (invoked && skill_name != "luoshen" && skill_name != "mingjie") {
        JsonArray msg;
    ...
    try {
        ...
        foreach (ServerPlayer *player, m_alivePlayers) {
            if (player->hasNullification()) {
                if (effect.card->tag["Fuji_tag"].toStringList().contains(player->objectName())
                        || effect.card->tag["Fuji_tag"].toStringList().contains("_ALL_PLAYERS")
                        || effect.card->tag["NoOffset"].toBool()) continue;
        ...
        if (repliedPlayer != NULL) {
            if (repliedPlayer != _m_AIraceWinner) {
                ...
                if (clientReply.size() > 0 && JsonUtils::isString(clientReply[0]))
                    _card = Card::Parse(clientReply[0].toString());
            } else {
                AI *ai = repliedPlayer->getAI();
                _card = ai->askForNullification(source_effect.card, from, to, positive);
            }
        }

        if (_card == NULL) return false;

        const Card *card = _card->validateInResponse(repliedPlayer);
                             ...
                             bool isRetrial, const QString &skill_name, bool isProvision)
{
    ...
    if (has_provided || !player->isAlive()) {
        _card = provided;
        if (player->isCardLimited(_card, method)) _card = NULL;
        provided = NULL;
        has_provided = false;
    } else {
        AI *ai = player->getAI();
        if (ai) {
            _card = ai->askForCard(pattern, prompt, data);
            if (_card && _card->isKindOf("DummyCard") && _card->subcardsLength() == 1)
                _card = Sanguosha->getCard(_card->getEffectiveId());
            if (_card && player->isCardLimited(_card, method)) _card = NULL;
            thread->delay();
        } else {
            JsonArray arg;
            arg << pattern;
    ...
    const Card *card = _card->validateInResponse(player, targets);
        ...
        if ((method == Card::MethodUse || method == Card::MethodResponse) && !isRetrial) {
            ...
            if (method == Card::MethodUse) {
                ...
                if (!isProvision) {
                    ...
                    _card->validateInResponseAfter(player, card);
                }
            }
...
const Card *Room::askForSinglePeach(ServerPlayer *player, ServerPlayer *dying)
{
    ...
    AI *ai = player->getAI();
    if (ai) {
        _card = ai->askForSinglePeach(dying);
        thread->delay();
    } else {
    ...
    if (_card == NULL || player->isCardLimited(_card, Card::MethodUse))
        return NULL;
...
void Room::startGame()
{
    ...
    foreach (ServerPlayer *player, m_players) {
        if (!Config.EnableBasara && (mode == "06_3v3" || mode == "02_1v1" || mode == "06_XMode" || mode == "03_1v2" || !player->isLord()))
            broadcastProperty(player, "general");
...
void Room::moveCardsAtomic(QList<CardsMoveStruct> cards_moves, bool forceMoveVisible)
{
    ...
    for (int i = 0; i < cards_moves.size(); i++) {
        ...
        for (int j = 0; j < cards_move.card_ids.size(); j++) {
            ...
            case Player::DiscardPile:
                discardpile.append(card_id);
                break;
            case Player::DrawPile:
                drawpile_up.append(card_id);
                break;
            case Player::DrawPileBottom:
                drawpile_bottom.append(card_id);
                break;
            case Player::PlaceSpecial:
                tablecards.append(card_id);
                break;
            default:
                break;
            }
...
void Room::activate(ServerPlayer *player, CardUseStruct &card_use)
{
    ...
    if (ai) {
        ...
        if (card_use.card)
            card_use.card = Card::Parse(card_use.card->toString());
...
bool Room::askForDiscard(ServerPlayer *player, const QString &reason, int discard_num, int min_num,
                         bool optional, bool include_equip, const QString &prompt, const QString &pattern, const QString &pattern2)
{
    if (!player->isAlive())
        return false;
    tryPause();
    ...
    foreach (int id, to_discard) {
        if (!Sanguosha->matchExpPattern(pattern2, player, Sanguosha->getCard(id))) {
            cheak = false;
            break;
        }
    }
...
AskForMoveCardsStruct Room::askForYanjiao(ServerPlayer *zhuge, const QList<int> &cards, bool visible, const QString &reason, const QString &pattern)
{
    QList<int> top_cards, bottom_cards;
    bool success = false;
    tryPause();
    notifyMoveFocus(zhuge, S_COMMAND_SKILL_YANJIAO);
    ...
    if (ai) {
        ...
        CardChooseArgs << reason;
        CardChooseArgs << pattern;
        success = doRequest(zhuge, S_COMMAND_SKILL_YANJIAO, CardChooseArgs, true);
        if (success) {
            JsonArray clientReply = zhuge->getClientReply().value<JsonArray>();
            if (clientReply.size() == 2) {
                success &= JsonUtils::tryParse(clientReply[0], top_cards);
    ...
    QVariant decisionData = QVariant::fromValue(reason + "yanjiao:" + zhuge->objectName() + ":" + IntList2StringList(top_cards).join("+") + ":" + IntList2StringList(bottom_cards).join("+"));
    thread->trigger(ChoiceMade, this, zhuge, decisionData);
    AskForMoveCardsStruct returns;
    returns.top = top_cards;
    returns.bottom = bottom_cards;
