    ...
    player_count(1) // Self is not included!! Be care!!!
{
    ...
    m_interactions[S_COMMAND_SKILL_YANJIAO] = &Client::askForYanjiao;
...
void Client::askForYanjiao(const QVariant &arg)
{
    JsonArray args = arg.value<JsonArray>();
    if (args.isEmpty())
        return;
    ...
    QList<int> card_ids;
    JsonUtils::tryParse(up, card_ids);
    m_isDiscardActionRefusable = true;
    m_canDiscardEquip = true;

    emit yanjiaochoose(card_ids, reason, func);
    setStatus(AskForYanjiao);
...
void Client::onPlayerReplyYanjiao(const QList<int> &up_cards, const QList<int> &down_cards)
{
    JsonArray decks;
    decks << JsonUtils::toJsonArray(up_cards);
    decks << JsonUtils::toJsonArray(down_cards);

    replyToServer(S_COMMAND_SKILL_YANJIAO, decks);
