...
class YaomingDiscard : public ViewAsSkill
{
public:
    YaomingDiscard() : ViewAsSkill("yaoming_discard")
    {
        response_pattern = "@@yaoming_discard!";
    }
...
public:
    ...
    virtual bool yaomingCheak(ServerPlayer *source, ServerPlayer *target) const
    {
        int x = source->getHandcardNum(), y = target->getHandcardNum();
        return (x < y && !source->hasFlag("Yaoming1Invoked")) || (x > y && !source->hasFlag("Yaoming2Invoked"))
                || (x == y && !source->hasFlag("Yaoming3Invoked") && !target->isNude());
    }
    ...
    virtual bool effect(TriggerEvent , Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        ...
        if (target) {
            target->broadcastSkillInvoke(objectName());
            int x = player->getHandcardNum(), y = target->getHandcardNum();
            if (x < y) {
                room->setPlayerFlag(player, "Yaoming1Invoked");
                ...
                room->setPlayerFlag(player, "Yaoming2Invoked");
                target->drawCards(1, objectName());
            } else if (x == y) {
                room->setPlayerFlag(player, "Yaoming3Invoked");
                if (target->canDiscard(target, "he")) {
                    QList<int> all_cards = target->forceToDiscard(10086, true);
                    QList<int> to_discard = target->forceToDiscard(1, true);
                    if (all_cards.length() > 1){
                        const Card *card = room->askForCard(target, "@@yaoming_discard!", "@yaoming-discard", data, Card::MethodNone);
                        if (card != NULL)
                    ...
                    if (!to_discard.isEmpty()) {
                        ...
                        target->drawCards(to_discard.length(), objectName());
                    }
