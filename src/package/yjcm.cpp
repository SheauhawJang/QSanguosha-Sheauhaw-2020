...
void XianzhenCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    if (effect.from->pindian(effect.to, "xianzhen", NULL)) {
        ...
        QStringList assignee_list2 = effect.from->property("IngoreArmor").toString().split("+");
        assignee_list << effect.to->objectName();
        room->setPlayerProperty(effect.from, "IngoreArmor", assignee_list2.join("+"));
    } else {
        room->setPlayerFlag(effect.from, "XianzhenFailure");
...
public:
    ...
    virtual void record(TriggerEvent triggerEvent, Room *room, ServerPlayer *gaoshun, QVariant &) const
    {
        if (triggerEvent == EventPhaseStart && gaoshun->getPhase() == Player::NotActive) {
            QStringList assignee_list = gaoshun->property("xianzhen_targets").toString().split("+");
            QStringList assignee_list2 = gaoshun->property("IngoreArmor").toString().split("+");

            foreach (QString name, assignee_list)
                assignee_list2.removeOne(name);

            room->setPlayerProperty(gaoshun, "xianzhen_targets", QVariant());
            room->setPlayerProperty(gaoshun, "IngoreArmor", assignee_list2.join("+"));
        }
    }
...
class XianzhenHideCard : public HideCardSkill
{
public:
    XianzhenHideCard() : HideCardSkill("#xianzhen-hidecard")
    {
    }
    virtual bool isCardHided(const Player *player, const Card *card) const
    {
        return player->hasFlag("XianzhenFailure") && card->isKindOf("Slash");
    }
};
...
public:
    ...
    virtual bool effect(TriggerEvent , Room *, ServerPlayer *player, QVariant &, ServerPlayer *) const
    {
        if (player->askForSkillInvoke(this)) {
            player->broadcastSkillInvoke(objectName());
            player->fillHandCards(player->getLostHp(), objectName());
        }
        return false;
    }
};
...
public:
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        ...
        if (player->askForSkillInvoke(this, QVariant::fromValue(target))) {
            ...
            room->preventDamage(damage);
            if (player->canGetCard(target, "hej")) {
                int card_id = room->askForCardChosen(player, target, "hej", objectName(), false, Card::MethodGet);
...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *player) const
    {
        ...
        if (target) {
            player->broadcastSkillInvoke(objectName());
            int x = qMax(target->getHandcardNum() - target->getHp(), 1);
            ...
            delete dummy;
            if (y > 0) {
                if (!room->askForDiscard(player, objectName(), y, y, true, true, "zhenjun-discard::"+target->objectName()))
                    target->drawCards(y, objectName());
            }
        }
        return false;
    }
    ...
    virtual bool effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *, QVariant &data, ServerPlayer *player) const
    {
        if (triggerEvent == TargetSpecified) {
            ...
            if (player->askForSkillInvoke(this, QVariant::fromValue(to))) {
                ...
                if (n > 0) {
                    QList<int> cards = room->askForCardsChosen(player, to, "he", objectName(), 1, n), equips;
                    to->addToPile("army", cards);
                    bool draw = false;
                    foreach (int id, cards) {
    ...
    : Package("YJCM")
{
    ...
    gaoshun->addSkill(new XianzhenHideCard);
    gaoshun->addSkill(new Jinjiu);
    related_skills.insertMulti("xianzhen", "#xianzhen-target");
    related_skills.insertMulti("xianzhen", "#xianzhen-hidecard");
