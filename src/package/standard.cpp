...
public:
    ...
    {
        events << CardsMoveOneTime << Death << DamageCaused << PreDamageDone
               << CardUsed << PreCardResponded << CardFinished << CardResponded << TargetSpecified << TargetConfirmed;
        global = true;
    }
    ...
    virtual void record(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == CardsMoveOneTime && !room->getTag("FirstRound").toBool()) {
            ...
            if (current && current->getPhase() != Player::NotActive) {
                ...
                room->setTag("GlobalDeathCount", room->getTag("GlobalDeathCount").toInt()+1);
            }
            ...
            ServerPlayer *current = room->getCurrent();
            if (current && current->getPhase() != Player::NotActive) {
                if (!player->hasFlag("JiedaoHadCausedDamge")) {
                    room->setPlayerFlag(player, "JiedaoHadCausedDamge");
                    damage.flags << "FirstCauseDamge";
                }
            }
            ...
            if (damage.card) {
                ...
                damaged_datas << data;
                room->setTag("GlobalCardDamagedTag", QVariant::fromValue(damaged_datas));
            ...
            if (current && current->getPhase() != Player::NotActive) {
                ...
                if (damage.from)
                    damaged_tag << damage.from->objectName();
                player->tag["GlobalRoundDamagedTag"] = QVariant::fromValue(damaged_tag);
            ...
            if (card && card->getTypeId() != Card::TypeSkill) {
                ...
                if (current && current->getPhase() != Player::NotActive) {
                    if (is_use) {
                        ...
                        if (current == player)
                            card->setTag("XuJingNumber", player->getCardUsedTimes(".")+1);
                    ...
                    if (current->getPhase() == Player::Play) {
                        if (is_use) {
                            ...
                            if (card->isKindOf("Slash"))
                                room->setPlayerFlag(player, "paoxiaoHadUseSlash");


                            QVariantList targets_list = player->tag["PlayCardTargets"].toList();
                            ...
                            player->tag["PlayCardTargets"] = targets_list;
                ...
                if (is_use && card->isKindOf("Analeptic") && !card->hasFlag("UsedBySecondWay"))
            ...
            if (use.card && use.card->getTypeId() != Card::TypeSkill) {
                ...
                if (player->getPhase() != Player::NotActive) {
                    foreach (ServerPlayer *p, use.to) {
                        if (p != player)
                            room->setTag("TargetOtherPlayers", true);
                    }
            ...
            if (current && current->getPhase() != Player::NotActive) {
                if (use.card->isKindOf("Slash") || use.card->isKindOf("Duel")) {
                    if (!player->hasFlag("FuyinHadTargeted")) {
                        room->setPlayerFlag(player, "FuyinHadTargeted");
                        use.card->tag["FuyinCanInvoke"] = true;
                    } else
                        use.card->tag["FuyinCanInvoke"] = false;
                }
            }
        ...
        if (triggerEvent == EventPhaseStart) {
            ...
            if (player->getPhase() == Player::NotActive) {
                foreach (ServerPlayer *p, room->getAlivePlayers()) {
                    ...
                    room->setPlayerMark(p, "GlobalDamageCount", 0);
                    ...
                    room->removeTag("TargetOtherPlayers");

                    p->tag.remove("GlobalRoundDamagedTag");

                    room->removeTag("GlobalDeathCount");
