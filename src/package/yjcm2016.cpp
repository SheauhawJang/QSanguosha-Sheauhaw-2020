...
void TaoluanCard::validateAfter(CardUseStruct &card_use) const
{
    ServerPlayer *zhangrang = card_use.from;
    Room *room = zhangrang->getRoom();
    const Card *taoluan_card = card_use.card;

    if (taoluan_card == NULL) return;

    ServerPlayer *target = room->askForPlayerChosen(zhangrang, room->getOtherPlayers(zhangrang), "taoluan", "@taoluan-choose");
    QString type_name[4] = { QString(), "BasicCard", "TrickCard", "EquipCard" };
    QStringList types;
    types << "BasicCard" << "TrickCard" << "EquipCard";
    types.removeOne(type_name[taoluan_card->getTypeId()]);
    const Card *card = room->askForCard(target, types.join(",") + "|.|.|.",
            "@taoluan-give:" + zhangrang->objectName() + "::" + taoluan_card->getType(), QVariant(), Card::MethodNone);
    if (card) {
        CardMoveReason reason(CardMoveReason::S_REASON_GIVE, target->objectName(), zhangrang->objectName(), "taoluan", QString());
...
class TaoluanViewAsSkill : public OneCardViewAsSkill
{
public:
    TaoluanViewAsSkill() : OneCardViewAsSkill("taoluan")
    {
        filter_pattern = ".";
    ...
    bool isEnabledAtResponse(const Player *player, const QString &pattern) const
    {
        ...
        if (player->hasFlag("Global_Dying") || player->hasFlag("TaoluanInvalid")) return false;
...
public:
    ...
    {
        view_as_skill = new TaoluanViewAsSkill;
    }
