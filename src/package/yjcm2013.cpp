...
public:
    ...
    {
        return TriggerSkill::triggerable(target) && target->getPhase() == Player::Finish && target->getCardUsedTimes(".") >= target->getHp();
    }
...
public:
    ...
    {
        CardUseStruct use = data.value<CardUseStruct>();
        if (room->askForCard(guanping, "..", "@longyin", data, objectName())) {
            if (use.m_addHistory) {
...
public:
    ...
    virtual const Card *viewAs(const QList<const Card *> &cards) const
    {
        if (cards.length() != Self->getMark("danshou")) return NULL;
...
public:
    ...
    {
        TriggerList skill_list;
        if (triggerEvent == EventPhaseStart && player->getPhase() == Player::Finish && player->isAlive()) {
    ...
    virtual bool effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *zhuran) const
    {
        if (triggerEvent == EventPhaseStart) {
            int x = player->getHandcardNum();
            if (x > 0) {
                room->setPlayerMark(zhuran, "danshou", x);
                QString prompt = "@danshou-discard::" + player->objectName() + ":" + QString::number(x);
                const Card *card = room->askForCard(zhuran, "@@danshou", prompt, data, Card::MethodNone);
                room->setPlayerMark(zhuran, "danshou", 0);
                if (card) {
                    LogMessage log;
                    ...
                    log.arg = objectName();
                    room->sendLog(log);
                    room->notifySkillInvoked(zhuran, objectName());
                    zhuran->broadcastSkillInvoke(objectName());
                    room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, zhuran->objectName(), player->objectName());

                    CardMoveReason reason(CardMoveReason::S_REASON_THROW, zhuran->objectName(), QString(), objectName(), QString());
                    room->moveCardTo(card, zhuran, NULL, Player::DiscardPile, reason);

                    room->damage(DamageStruct(objectName(), zhuran, player));
                }
            } else {
                if (zhuran->askForSkillInvoke(this, "prompt1::" + player->objectName())) {
                    zhuran->broadcastSkillInvoke(objectName());
                    room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, zhuran->objectName(), player->objectName());
                    room->damage(DamageStruct(objectName(), zhuran, player));
                }
            ...
            int x = zhuran->getMark("DanshouTimes");
            if (zhuran->askForSkillInvoke(this, "prompt2:::" + QString::number(x))) {
                zhuran->broadcastSkillInvoke(objectName());
                zhuran->setFlags("DanshouUsed");
                zhuran->drawCards(x, objectName());
            }
        }
        return false;
    }
...
void MiejiCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    if (room->askForDiscard(effect.to, "mieji", 1, 1, false, true, "@mieji-discard1", ".", "^TrickCard"))
        room->askForDiscard(effect.to, "mieji", 1, 1, false, true, "@mieji-discard2", "^TrickCard");
}
