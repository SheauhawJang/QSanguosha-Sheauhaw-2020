...
public:
    ...
    virtual void record(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &datas) const
    {
        if (triggerEvent == CardsMoveOneTime) {
            ...
            QVariantList guzhengOther = room->getTag("GuzhengOther").toList();
            ...
            room->setTag("GuzhengOther", guzhengOther);
        } else if (triggerEvent == EventPhaseChanging) {
            PhaseChangeStruct change = datas.value<PhaseChangeStruct>();
    ...
    virtual TriggerList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &) const
    {
        ...
        if (triggerEvent == EventPhaseEnd && player->getPhase() == Player::Discard) {
            if (player->isDead())
                return list;
            QVariantList guzheng_cardsToGet = room->getTag("GuzhengToGet").toList();
            QVariantList guzheng_cardsOther = room->getTag("GuzhengOther").toList();
...
public:
    ...
    {
        if (triggerEvent == EventPhaseChanging && liushan->askForSkillInvoke(this)) {
...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *liushan) const
    {
        ...
        if (room->changeMaxHpForAwakenSkill(liushan, 1)) {
            ...
            room->handleAcquireDetachSkills(liushan, "jijiang|sishu");
        }
...
class Sishu : public PhaseChangeSkill
{
public:
    Sishu() : PhaseChangeSkill("sishu")
    {
