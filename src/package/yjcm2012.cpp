...
public:
    ...
    virtual void record(TriggerEvent , Room *room, ServerPlayer *player, QVariant &) const
    {
        if (player->getPhase() == Player::NotActive) {
            ...
            foreach (ServerPlayer *p, players) {
                room->removePlayerTip(p, "#qianxi_black");
                room->removePlayerTip(p, "#qianxi_red");
            }
        }
    }
    ...
    virtual bool onPhaseChange(ServerPlayer *target) const
    {
        ...
        if (target->askForSkillInvoke(objectName())) {
            ...
            if (c == NULL) {
                int id = target->forceToDiscard(1, true).first();
                c = Card::Parse(Sanguosha->getCard(id)->toRealString());
            ...
            if (!to_choose.isEmpty()) {
                ...
                room->setPlayerCardLimitation(victim, "use,response", QString(".|%1|.|hand").arg(color), true);
                LogMessage log;
                log.type = "#Qianxi";
                log.from = victim;
...
public:
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *liaohua, QVariant &data, ServerPlayer *) const
    {
        if (liaohua->askForSkillInvoke(this, data)) {
            ...
            liaohua->fillHandCards(x, objectName());
