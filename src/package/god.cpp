...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *shenlvmeng) const
    {
        ...
        AskForMoveCardsStruct result = room->askForMoveCards(shenlvmeng, card_ids, QList<int>(), true, objectName(), "differentsuit", suits.size(), 4, false, true);
...
class QixingExchange : public ViewAsSkill
{
public:
    QixingExchange() : ViewAsSkill("qixing_exchange")
    {
        response_pattern = "@@qixing_exchange";
        expand_pile = "cardcontainer";
    }
    ...
    virtual const Card *viewAs(const QList<const Card *> &cards) const
    {
        ...
        QList<int> card_ids = Self->getPile("stars");
        int i = 0, j = 0;
        foreach (const Card *c, cards) {
            if (card_ids.contains(c->getEffectiveId()))
                i++;
            else
                j++;
        }
...
public:
    ...
    virtual bool effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *, QVariant &, ServerPlayer *shenzhugeliang) const
    {
        QList<int> subcards;
        ...
        if (triggerEvent == TurnStart) {
            ...
            if (shenzhugeliang->isKongcheng() || shenzhugeliang->getPile("stars").isEmpty()) return false;

            room->fillAG(shenzhugeliang->getPile("stars"), QList<ServerPlayer *>() << shenzhugeliang);

            const Card *card = room->askForCard(shenzhugeliang, "@@qixing_exchange", "@qixing-exchange", QVariant(), Card::MethodNone);
            ...
            if (shenzhugeliang->askForSkillInvoke(objectName())) {
                shenzhugeliang->broadcastSkillInvoke(objectName());
        ...
        QList<int> pile = shenzhugeliang->getPile("stars");
        ...
        shenzhugeliang->addToPile("stars", to_pile, false);

        DummyCard *to_handcard_x = new DummyCard(to_handcard);
        CardMoveReason reason(CardMoveReason::S_REASON_EXCHANGE_FROM_PILE, shenzhugeliang->objectName());
        room->obtainCard(shenzhugeliang, to_handcard_x, reason, false);
...
{
    QStringList assignee_list = effect.from->tag["KuangfengTarget"].toStringList();
    assignee_list << effect.to->objectName();
    effect.from->tag["KuangfengTarget"] = QVariant::fromValue(assignee_list);
    effect.from->getRoom()->setPlayerMark(effect.to, "@gale", 1);
}
...
public:
    ...
    virtual void record(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == EventPhaseStart && player->getPhase() == Player::RoundStart)
            player->tag.remove("KuangfengTarget");
        else if (triggerEvent == Death) {
            DeathStruct death = data.value<DeathStruct>();
        ...
        foreach (ServerPlayer *p1, players) {
            if (p1->getMark("@gale") > 0) {
                bool clear = true;
                foreach (ServerPlayer *p2, players) {
                    QStringList assignee_list = p2->tag["KuangfengTarget"].toStringList();
                    if (assignee_list.contains(p1->objectName())) {
                        clear = false;
                        break;
                    }
                }
                if (clear)
                    room->setPlayerMark(p1, "@gale", 0);
            }
        }
    ...
    virtual TriggerList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == EventPhaseStart && player->getPhase() == Player::Finish) {
            ...
            if (damage.nature == DamageStruct::Fire) {
                QList<ServerPlayer *> players = room->getAlivePlayers();
                foreach (ServerPlayer *p, players) {
                    QStringList assignee_list = p->tag["KuangfengTarget"].toStringList();
                    if (assignee_list.contains(player->objectName())) {
                        skill_list.insert(p, QStringList("kuangfeng!"));
                    }
                }
            }
        }
    ...
    virtual bool effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        if (triggerEvent == EventPhaseStart) {
            room->askForUseCard(player, "@@kuangfeng", "@kuangfeng");
        }
...
{
    QStringList assignee_list = effect.from->tag["DawuTarget"].toStringList();
    assignee_list << effect.to->objectName();
    effect.from->tag["DawuTarget"] = QVariant::fromValue(assignee_list);
    effect.from->getRoom()->setPlayerMark(effect.to, "@fog", 1);
}
...
public:
    ...
    virtual void record(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        foreach (ServerPlayer *p1, players) {
            if (p1->getMark("@fog") > 0) {
                bool clear = true;
                foreach (ServerPlayer *p2, players) {
                    QStringList assignee_list = p2->tag["DawuTarget"].toStringList();
                    if (assignee_list.contains(p1->objectName())) {
                        clear = false;
                        break;
                    }
    ...
    virtual TriggerList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == EventPhaseStart && player->getPhase() == Player::Finish) {
            ...
            if (damage.nature != DamageStruct::Thunder) {
                QList<ServerPlayer *> players = room->getAlivePlayers();
                foreach (ServerPlayer *p, players) {
                    QStringList assignee_list = p->tag["DawuTarget"].toStringList();
                    if (assignee_list.contains(player->objectName())) {
                        skill_list.insert(p, QStringList("dawu!"));
                    }
                }
            }
        }
    ...
    {
        if (triggerEvent == EventPhaseStart) {
            room->askForUseCard(player, "@@dawu", "@dawu");
        }
        if (triggerEvent == DamageInflicted) {
            DamageStruct damage = data.value<DamageStruct>();
            LogMessage log;
...
LonghunCard::LonghunCard()
{
    will_throw = false;
    handling_method = Card::MethodNone;
}

bool LonghunCard::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const
{
    FireSlash *slash = new FireSlash(getSuit(), getNumber());
    slash->addSubcards(getSubcards());
    slash->setSkillName("longhun");
    return slash && slash->targetFilter(targets, to_select, Self) && !Self->isProhibited(to_select, slash, targets);
}

bool LonghunCard::targetFixed() const
{
    if (Sanguosha->currentRoomState()->getCurrentCardUseReason() == CardUseStruct::CARD_USE_REASON_RESPONSE)
        return true;

    return user_string != "fire_slash";
}

bool LonghunCard::targetsFeasible(const QList<const Player *> &targets, const Player *Self) const
{
    FireSlash *slash = new FireSlash(getSuit(), getNumber());
    slash->addSubcards(getSubcards());
    slash->setSkillName("longhun");
...
const Card *LonghunCard::validate(CardUseStruct &card_use) const
{
    return validateInResponse(card_use.from);
}

const Card *LonghunCard::validateInResponse(ServerPlayer *) const
{
...
void LonghunCard::validateInResponseAfter(ServerPlayer *zhaoyun, const Card *card) const
{
    if ((card->isKindOf("Jink") || card->isKindOf("Nullification")) && card->subcardsLength() > 1) {
        Room *room = zhaoyun->getRoom();
        ServerPlayer *current = room->getCurrent();
        if (current && zhaoyun->canDiscard(current, "he")) {
            int to_throw = room->askForCardChosen(zhaoyun, current, "he", "longhun", false, Card::MethodDiscard);
            room->throwCard(to_throw, current, zhaoyun);
        }
    }
}
...
bool Longhun::hasAvailable(const Player *player) const
{
...
bool Longhun::isEnabledAtPlay(const Player *player) const
{
    ...
    peach->setSkillName("longhun");
    return hasAvailable(player) || peach->isAvailable(player) || Slash::IsAvailable(player);
}
...
bool Longhun::viewFilter(const QList<const Card *> &selected, const Card *card) const
{
    ...
    case CardUseStruct::CARD_USE_REASON_RESPONSE_USE: {
        ...
        } else if (pattern == "nullification" && card->getSuit() == Card::Spade) {
            Nullification *nullification = new Nullification(Card::SuitToBeDecided, -1);
            nullification->addSubcards(selected);
            nullification->addSubcard(card->getEffectiveId());
            nullification->deleteLater();
            return !Self->isLocked(nullification);
        }

        else if (pattern.contains("peach") && card->getSuit() == Card::Heart) {
            Peach *peach = new Peach(Card::SuitToBeDecided, -1);
            peach->addSubcards(selected);
            peach->addSubcard(card->getEffectiveId());
            peach->deleteLater();
            return !Self->isLocked(peach);
        } else if (pattern == "slash" && card->getSuit() == Card::Heart) {
            FireSlash *slash = new FireSlash(Card::SuitToBeDecided, -1);
            slash->addSubcards(selected);
            slash->addSubcard(card->getEffectiveId());
...
const Card *Longhun::viewAs(const QList<const Card *> &cards) const
{
    ...
    if (!card_name.isEmpty()) {
        LonghunCard *skill_card = new LonghunCard;
...
int Longhun::getEffHp(const Player *) const
{
    return 2;
}
...
void PoxiCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    room->fillAG(handcards, QList<ServerPlayer *>() << source);
...
class Chuyuan : public TriggerSkill
{
public:
    Chuyuan() : TriggerSkill("chuyuan")
    {
        events << Damaged;
    }
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *target, QVariant &, ServerPlayer *shencaopi) const
    {
        if (shencaopi->askForSkillInvoke(objectName(), QVariant::fromValue(target))) {
            shencaopi->broadcastSkillInvoke(objectName());
            room->doAnimate(QSanProtocol::S_ANIMATE_INDICATE, shencaopi->objectName(), target->objectName());
            target->drawCards(1, objectName());
            if (!target->isKongcheng()) {
                const Card *card = room->askForExchange(target, objectName(), 1, 1, false, "@chuyuan-push:" + shencaopi->objectName());
                shencaopi->addToPile("crown", card);
            }
        }
        return false;
    }
...
class ChuyuanMaxCards : public MaxCardsSkill
{
public:
    ChuyuanMaxCards() : MaxCardsSkill("#chuyuan-maxcards")
    {
    ...
    virtual int getExtra(const Player *target) const
    {
        if (target->hasSkill("chuyuan"))
            return target->getPile("crown").length();
        else
            return 0;
    }
};
...
public:
    ...
    virtual bool triggerable(const ServerPlayer *target) const
    {
        return PhaseChangeSkill::triggerable(target)
               ...
               && target->getPhase() == Player::Start
               && target->getPile("crown").length() > 2;
    }

    virtual bool onPhaseChange(ServerPlayer *shencaopi) const
    {
        Room *room = shencaopi->getRoom();
        room->sendCompulsoryTriggerLog(shencaopi, objectName());

        shencaopi->broadcastSkillInvoke(objectName());

        room->setPlayerMark(shencaopi, objectName(), 1);
        if (room->changeMaxHpForAwakenSkill(shencaopi) && shencaopi->getMark(objectName()) == 1) {
            DummyCard *dummy = new DummyCard(shencaopi->getPile("crown"));
            CardMoveReason reason(CardMoveReason::S_REASON_EXCHANGE_FROM_PILE, shencaopi->objectName(), objectName(), QString());
            room->obtainCard(shencaopi, dummy, reason, false);
            room->handleAcquireDetachSkills(shencaopi, "jianxiong|tianxing");
        }
        return false;
    }
};
...
public:
    ...
    virtual bool onPhaseChange(ServerPlayer *shencaopi) const
    {
        ...
        if (room->changeMaxHpForAwakenSkill(shencaopi) && shencaopi->getMark(objectName()) == 1) {
            ...
            CardMoveReason reason(CardMoveReason::S_REASON_EXCHANGE_FROM_PILE, shencaopi->objectName(), objectName(), QString());
            room->obtainCard(shencaopi, dummy, reason, false);
            room->detachSkillFromPlayer(shencaopi, "chuyuan");
            QStringList skill_list;
            if (!shencaopi->hasSkill("rende", true))
                skill_list << "rende";
            if (!shencaopi->hasSkill("zhiheng", true))
                skill_list << "zhiheng";
            if (!shencaopi->hasSkill("luanji", true))
                skill_list << "luanji";
            if (!shencaopi->hasSkill("fangquan", true))
                skill_list << "fangquan";
            if (!skill_list.isEmpty())
                room->acquireSkill(shencaopi, room->askForChoice(shencaopi, objectName(), skill_list.join("+"), QVariant(), "@tianxing-choose"));
        }
        return false;
    }
};
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *) const
    {
        ...
        if (player->getHandcardNum() % 2 == 0) {
            ...
            while (true) {
                ...
                ServerPlayer *target = room->askForPlayerChosen(player, targets, objectName(), "@shenfu2-choose", true);
                if (target) {
                    QStringList choices;
                    choices << "draw";
                    if (!target->isKongcheng())
                        choices << "discard";
                    QString choice = room->askForChoice(player, objectName(), choices.join("+"), QVariant(), "@shenfu-choice:" + target->objectName(), "draw+discard");
                    if (choice == "draw")
                        target->drawCards(1, objectName());
                    else if (player->canDiscard(target, "h"))
            ...
            while (true) {
                ServerPlayer *vic = room->askForPlayerChosen(player, room->getOtherPlayers(player), objectName(), "@shenfu1-choose", true);
                if (vic) {
                    room->damage(DamageStruct(objectName(), player, vic, 1, DamageStruct::Thunder));
                    if (vic->isAlive()) break;
                } else
                    break;
            }
        }
    ...
    : Package("god")
{
    ...
    General *shencaopi = new General(this, "shencaopi", "god", 6);
    shencaopi->addSkill(new Chuyuan);
    shencaopi->addSkill(new ChuyuanMaxCards);
    shencaopi->addSkill(new Dengji);
    shencaopi->addRelateSkill("tianxing");
    shencaopi->addRelateSkill("jianxiong");
    shencaopi->addRelateSkill("rende");
    shencaopi->addRelateSkill("zhiheng");
    shencaopi->addRelateSkill("luanji");
    shencaopi->addRelateSkill("fangquan");
    related_skills.insertMulti("chuyuan", "#chuyuan-maxcards");

    General *shenzhenji = new General(this, "shenzhenji", "god", 4, false);
    ...
    addMetaObject<LonghunCard>();
    addMetaObject<ZhanhuoCard>();
    addMetaObject<PoxiCard>();

    skills << new QixingExchange << new Jilve << new LongnuFire << new LongnuThunder << new PoxiChoose << new Tianxing;
}
