    ...
    virtual QStringList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer * &) const
    {
        ...
        if (card && card->getTypeId() != Card::TypeSkill && card->getHandlingMethod() == Card::MethodUse) {
            ...
            if (targets_list.length() > 1) {
                QStringList targets1 = targets_list.last().toStringList();
                QStringList targets2 = targets_list.at(targets_list.length()-2).toStringList();
...
{
    room->removePlayerMark(card_use.from, "@limit_garrison");
}
