...
public:
    ...
    {
        if (TriggerSkill::triggerable(player) && !player->hasFlag("shenxianUsed")) {
            ServerPlayer *current = room->getCurrent();
            if (current && current != player && current->getPhase() != Player::NotActive) {
                ...
                foreach(QVariant move_data, move_datas) {
                    ...
                    if (move.from && move.from->objectName() != player->objectName()
                        ...
                        foreach (QString card_str, move.cards) {
                            ...
                            if (card && card->getTypeId() == Card::TypeBasic && (move.from_places[i] == Player::PlaceHand || move.from_places[i] == Player::PlaceEquip)) {
                                return nameList();
                            }
                            i++;
                        }
...
public:
    ...
    virtual QStringList triggerable(TriggerEvent triggerEvent, Room *, ServerPlayer *player, QVariant &data, ServerPlayer *&) const
    {
        if (triggerEvent == EventPhaseStart && TriggerSkill::triggerable(player) && player->getPhase() == Player::Start)
            ...
            if (!player->getUseExtraTargets(use).isEmpty()) return nameList();
        }
        return QStringList();
    }
        ...
        if (use.card->isKindOf("Slash")) {
            ...
            foreach (ServerPlayer *caoang, caoangs) {
                if (caoang->distanceTo(player) < 2)
                    skill_list.insert(caoang, nameList());
            }
        }
        return skill_list;
    }
...
public:
    ...
    virtual TriggerList triggerable(TriggerEvent, Room *room, ServerPlayer *, QVariant &) const
    {
        ...
        foreach (ServerPlayer *zhugedan, zhugedans)
            skill_list.insert(zhugedan, nameList());
        return skill_list;
    }
...
public:
    ...
    bool onPhaseChange(ServerPlayer *zhugedan) const
    {
        ...
        zhugedan->fillHandCards(zhugedan->getMaxHp(), objectName());
...
public:
    ...
    virtual QStringList triggerable(TriggerEvent, Room *, ServerPlayer *player, QVariant &data, ServerPlayer *&) const
    {
        ...
        if (use.card->isNDTrick() && !use.card->isKindOf("Collateral") && !use.card->isKindOf("BeatAnother")) {
            if (!use.from->getUseExtraTargets(use).isEmpty()) return nameList();
...
public:
    ...
    {
        if (from->hasSkill("limu") && !from->getJudgingArea().isEmpty() && to) {
        ...
        if (use.card->isKindOf("Slash") && use.from && use.from->isAlive()) {
            ...
            foreach (ServerPlayer *qinmi, qinmis) {
                if (use.from->inMyAttackRange(qinmi) && !use.to.contains(qinmi) && !qinmi->isKongcheng())
                    skill_list.insert(qinmi, nameList());
            }
        }
        return skill_list;
    }
            ...
            if (suit <= Card::Diamond && x > 1) {
                ...
                foreach (ServerPlayer *panjun, panjuns) {
                    if (!panjun->isNude() && !panjun->hasFlag("GuanweiUsed"))
                        skill_list.insert(panjun, nameList());
                }
            }
        }
    ...
    virtual TriggerList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &) const
    {
        ...
        if (triggerEvent == TurnStart) {
            ...
            foreach (ServerPlayer *xizhicai, xizhicais)
                skill_list.insert(xizhicai, nameList());
...
public:
    ...
    virtual QStringList triggerable(TriggerEvent, Room *, ServerPlayer *wangcan, QVariant &data, ServerPlayer *&) const
    {
        ...
        if (!getSanwenCards(wangcan, data, false).isEmpty()) return nameList();
        return QStringList();
    }
...
class TongboExchange : public ViewAsSkill
{
public:
    TongboExchange() : ViewAsSkill("tongbo_exchange")
    {
        response_pattern = "@@tongbo_exchange";
    ...
    virtual const Card *viewAs(const QList<const Card *> &cards) const
    {
        ...
        QList<int> pile = Self->getPile("books");
        int i = 0, j = 0;
        foreach (const Card *c, cards) {
            if (pile.contains(c->getEffectiveId()))
                i++;
            else
                j++;
        }
...
public:
    ...
    virtual bool triggerable(const ServerPlayer *target) const
    {
        return TriggerSkill::triggerable(target) && target->getPile("books").length() > 0
               && !target->isNude() && target->getPhase() == Player::Draw;
    }

    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *caiyong, QVariant &) const
    {
        if (!caiyong->askForSkillInvoke(objectName())) return false;
        caiyong->broadcastSkillInvoke(objectName());

        QList<int> pile = caiyong->getPile("books");

        room->fillAG(pile, QList<ServerPlayer *>() << caiyong);

        const Card *card = room->askForCard(caiyong, "@@tongbo_exchange", "@tongbo-exchange", QVariant(), Card::MethodNone);
        ...
        if (card != NULL && card->subcardsLength() > 0) {
            ...
            if (pile.length() == 4) {
                ...
                room->askForRende(caiyong, pile, "tongbo", false, false, false, 4, 4,
                                  room->getOtherPlayers(caiyong), CardMoveReason(), "@tongbo-give", "books");
...
public:
    ...
    virtual TriggerList triggerable(TriggerEvent, Room *room, ServerPlayer *player, QVariant &) const
    {
        ...
        foreach (ServerPlayer *sunshangxiang, sunshangxiangs)
            skill_list.insert(sunshangxiang, nameList());
        return skill_list;
    }
...
void JuesiCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    if (room->askForDiscard(effect.to, "juesi", 1, 1, false, true, "@juesi-discard:" + source->objectName(), ".", "Slash") &&
            source->getHp() <= target->getHp()) {
        Duel *duel = new Duel(Card::NoSuit, 0);
        duel->setSkillName("_juesi");
        if (!source->isLocked(duel) && !source->isProhibited(target, duel))
...
public:
    ...
    virtual TriggerList triggerable(TriggerEvent, Room *room, ServerPlayer *, QVariant &) const
    {
        ...
        foreach (ServerPlayer *mizhu, mizhus)
            skill_list.insert(mizhu, nameList());
        return skill_list;
    }
...
public:
    ...
    TriggerList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == CardUsed || triggerEvent == CardResponded) {
            ...
            if (triggerEvent == CardUsed) {
                if (data.value<CardUseStruct>().from != player) return list;
                usecard = data.value<CardUseStruct>().card;
            } else if (triggerEvent == CardResponded) {
                CardResponseStruct resp = data.value<CardResponseStruct>();
                if (resp.m_who != player) return list;
                if (resp.m_isUse)
                    usecard = resp.m_card;
            }
            if (usecard && usecard->getTypeId() != Card::TypeSkill)
                foreach (ServerPlayer *p, room->getAlivePlayers())
                    if (TriggerSkill::triggerable(p))
                        foreach (QVariant data, p->tag["fuman_record"].toList()) {
                            QPair<int, ServerPlayer *> info = data.value<QPair<int, ServerPlayer *> >();
                            if (info.second == player && usecard->getSubcards().contains(info.first))
                                list[p].append("fuman!");
                        }
        }
        return list;
    ...
    void record(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == EventPhaseChanging) {
            ...
            foreach (ServerPlayer *p, room->getAlivePlayers()) {
                QVariantList list;
                foreach (QVariant adata, p->tag["fuman_record"].toList()) {
                    QPair<int, ServerPlayer *> info = adata.value<QPair<int, ServerPlayer *> >();
                    if (info.second != player)
                        list << adata;
                }
                p->tag["fuman_record"] = list;
            }
        }
    }
    ...
    bool effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *ask_who) const
    {
        ...
        log.arg = objectName();
        room->sendLog(log);
        ask_who->drawCards(1, "fuman");
        QVariantList list = ask_who->tag["fuman_record"].toList();
        for (int i = 0; i < list.length(); ++i) {
            QPair<int, ServerPlayer *> info = list[i].value<QPair<int, ServerPlayer *> >();
            if (info.second == player && usecard->getSubcards().contains(info.first)) {
                list.removeAt(i);
                break;
            }
        }
        ask_who->tag["fuman_record"] = list;
        return false;
    }
};
...
public:
            ...
            if (use.card && (use.card->isKindOf("Slash") || (use.card->getTypeId() == Card::TypeTrick && use.card->isBlack()))) {
                ...
                foreach (ServerPlayer *wenpin, wenpins) {
                    if (!wenpin->isNude() && wenpin->getHp() > player->getHp())
                        skill_list.insert(wenpin, nameList());
                }
            }
        } else if (triggerEvent == EventPhaseChanging) {
            PhaseChangeStruct change = data.value<PhaseChangeStruct>();
    ...
    {
        if (triggerEvent == EventPhaseChanging) {
            DummyCard *dummy = new DummyCard(ask_who->getPile("zhenwei_zheng"));
    ...
    virtual void record(TriggerEvent, Room *room, ServerPlayer *, QVariant &) const
    {
        ...
        foreach (ServerPlayer *p1, players) {
            ...
            foreach (ServerPlayer *p2, players) {
                if (p1 != p2 && p2->hasSkill("wengua", true)) {
                    has_skill = true;
                    break;
                }
            }
            if (has_skill && !p1->hasSkill("wengua_attach", true))
                room->attachSkillToPlayer(p1, "wengua_attach");
            else if (!has_skill && p1->hasSkill("wengua_attach", true))
                room->detachSkillFromPlayer(p1, "wengua_attach", true);
...
public:
    ...
    virtual TriggerList triggerable(TriggerEvent, Room *room, ServerPlayer *player, QVariant &) const
    {
        ...
        foreach (ServerPlayer *xushi, xushis) {
            ...
            skill_list.insert(xushi, nameList());
        }
        return skill_list;
    }
    ...
    : Package("spark3")
{
    ...
    skills << new DenglouUse << new JiaozhaoFirst << new JiaozhaoSecond << new TongboExchange;
}
