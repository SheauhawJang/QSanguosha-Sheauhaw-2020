...
public:
    ...
    virtual QStringList triggerable(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *&) const
    {
        if (player != NULL && player->isAlive() && player->hasLordSkill(objectName())) {
            ...
            if (!room->getLieges("wei", player).isEmpty())  return nameList();
        }
        return QStringList();
    }
...
public:
    ...
    virtual int getDrawNum(ServerPlayer *zhangliao, int n) const
    {
        ...
        room->setPlayerMark(zhangliao, "tuxi", n);
...
public:
    ...
    virtual bool effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        if (triggerEvent == EventPhaseStart) {
            ...
            if (!card_to_gotback.isEmpty() && room->askForSkillInvoke(player, "_luoyi", "prompt")) {
            ...
            if (to->isAlive() && to != first) {
                ...
                foreach (ServerPlayer *ganning, gannings) {
                    if (ganning->getMark(limit_mark) > 0)
                        skill_list.insert(ganning, nameList());
                }
                return skill_list;
            }
        }
...
public:
    ...
    virtual QStringList triggerable(TriggerEvent triggerEvent, Room *, ServerPlayer *gaodayihao, QVariant &data, ServerPlayer *&) const
    {
