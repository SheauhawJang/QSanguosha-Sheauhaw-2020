...
public:
    ...
    {
        if (from->hasSkill("wanglie") && from->getMark("GlobalPlayCardUsedTimes") == 0)
            return 1000;
        else
            return 0;
    }
...
class QianjieStatus : public StatusAbilitySkill
{
public:
    QianjieStatus() : StatusAbilitySkill("#qianjie-status")
    {
    ...
    {
        return !(player->hasSkill("qianjie") && setting);
    }

    virtual bool canPindian(const Player *player, const Player *from) const
    {
        return !player->hasSkill("qianjie");
    }
};
...
public:
    ...
    virtual TriggerList triggerable(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (triggerEvent == EventPhaseStart && player->getPhase() == Player::Finish && TriggerSkill::triggerable(player))
            ...
            foreach (ServerPlayer *haozhao, haozhaos) {
                if (haozhao_list.contains(haozhao->objectName())) {
                    int y = haozhao->getHandcardNum();
                    if (y < x || qMin(5, y) > x)
    ...
    virtual bool effect(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer *ask_who) const
    {
        ...
        if (triggerEvent == EventPhaseStart) {
            ...
            target = player;
        if (target) {
            int x = target->getHandcardNum(), y = ask_who->getHandcardNum();

            if (x > y)
                room->askForDiscard(target, objectName(), x - y, x - y, false, false, "@zhengu-discard");
            else
                target->fillHandCards(qMin(5, y), objectName());
        }
        return false;
    }
};
...
public:
    ...
    virtual bool effect(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data, ServerPlayer *) const
    {
        ...
        ServerPlayer *target = room->askForPlayerChosen(player, targets, objectName(), "@zhengrong-target", true, true);
        if (target) {
            player->broadcastSkillInvoke(objectName());
...
void QingceCard::extraCost(Room *room, const CardUseStruct &card_use) const
{
    CardMoveReason reason(CardMoveReason::S_REASON_EXCHANGE_FROM_PILE, card_use.from->objectName());
    room->obtainCard(card_use.from, this, reason, false);
}

void QingceCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const
{
    room->askForDiscard(source, "qingce", 1, 1, false, false, "@qingce-discard");

    QList<ServerPlayer *> targets, all = room->getAlivePlayers();
    ...
    ServerPlayer *vic = room->askForPlayerChosen(source, targets, "qingce", "@qingce-target");
    int id = room->askForCardChosen(source, vic, "ej", "qingce", false, Card::MethodDiscard);
    room->throwCard(id, vic, source);
    ...
    : Package("thunder")
{
    ...
    lukang->addSkill(new QianjieStatus);
    ...
    related_skills.insertMulti("qianjie", "#qianjie-status");
