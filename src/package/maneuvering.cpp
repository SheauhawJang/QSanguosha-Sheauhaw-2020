...
bool IronChain::targetRated(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const
{
    return targets.length() < 2 + Sanguosha->correctCardTarget(TargetModSkill::ExtraTarget, Self, this, to_select) && !Self->isCardLimited(this, Card::MethodUse);
}
